...
def shallow_pca(AD_name, shallow_name, dataset, representation, nbr_modes, pca_explained_var, nbr_seeds, train_ratio,
                     nbr_pulse_per_scan, nbr_targets, date_string, nbr_projections=1000, estimator="mean",
                     unit_norm=True, n_components_pca=30, n_components_nnpca=10):
    ...
    for exp_index, seed in enumerate(range(nbr_seeds)):
        ...
        if representation=="sp":
            train_sig = radar_dataset.train_sps_norm_2D
            complete_train_sig = radar_dataset.complete_train_sps_norm_2D
            val_sig = radar_dataset.val_sps_norm_2D
            test_sig = radar_dataset.test_sps_norm_2D

            pca = PCA(n_components=pca_explained_var, svd_solver='full')

            train_sig_reduced = pca.fit_transform(train_sig.view(train_sig.size()[0], -1))
            complete_train_sig_reduced = pca.transform(complete_train_sig.view(complete_train_sig.size()[0], -1))
            val_sig_reduced = pca.transform(val_sig.view(val_sig.size()[0], -1))
            test_sig_reduced = pca.transform(test_sig.view(test_sig.size()[0], -1))
        elif representation=="cov":
            train_sig = radar_dataset.train_spds_norm_1Dut
            complete_train_sig = radar_dataset.complete_train_spds_norm_1Dut
            val_sig = radar_dataset.val_spds_norm_1Dut
            test_sig = radar_dataset.test_spds_norm_1Dut

            pca = PCA(n_components=pca_explained_var, svd_solver='full')

            train_sig_reduced = pca.fit_transform(train_sig)
            complete_train_sig_reduced = pca.transform(complete_train_sig)
            val_sig_reduced = pca.transform(val_sig)
            test_sig_reduced = pca.transform(test_sig)
        elif representation == "nn-pca-sp":
            train_sig = radar_dataset.train_sps_norm_2D
            complete_train_sig = radar_dataset.complete_train_sps_norm_2D
            val_sig = radar_dataset.val_sps_norm_2D
            test_sig = radar_dataset.test_sps_norm_2D
            ...
            val_sig_reduced = pca.transform(val_sig.view(val_sig.size()[0], -1))
            test_sig_reduced = pca.transform(test_sig.view(test_sig.size()[0], -1))
        elif representation=="nn-pca-cov":
            train_sig = radar_dataset.train_spds_norm_1Dut
            complete_train_sig = radar_dataset.complete_train_spds_norm_1Dut
            val_sig = radar_dataset.val_spds_norm_1Dut
            test_sig = radar_dataset.test_spds_norm_1Dut
            ...
            raise ValueError("Unsupported representation type. Use sp for doppler signature or cov for covariance matrix.")
        ...
        if shallow_name=="IF":
            ...
            clf = Norm_Negated_PCA(n_components=n_components_nnpca)
        else:
...
if __name__ == '__main__':
    ...
    representation = str(sys.argv[4])
    nbr_modes = int(sys.argv[5])
    pca_explained_var = float(sys.argv[6])
    nbr_seeds = int(sys.argv[7])
    train_ratio = float(sys.argv[8])
    nbr_pulse_per_scan = int(sys.argv[9])
    nbr_targets = int(sys.argv[10])
    if shallow_name == "RPO":
        ...
        n_components_pca = int(sys.argv[12])
        n_components_nnpca = int(sys.argv[13])
    else:
        experiment_name = str(sys.argv[11])
    ...
    if str(sys.argv[2]) == "RPO":
        results_df = shallow_pca(AD_name=AD_name, shallow_name=shallow_name, dataset=dataset, representation=representation, nbr_modes=nbr_modes,
                                      pca_explained_var=pca_explained_var, nbr_seeds=nbr_seeds, train_ratio=train_ratio,
    ...
    elif str(sys.argv[2]) == "NN-PCA":
        results_df = shallow_pca(AD_name=AD_name, shallow_name=shallow_name, dataset=dataset, representation=representation, nbr_modes=nbr_modes,
                                      pca_explained_var=pca_explained_var, nbr_seeds=nbr_seeds, train_ratio=train_ratio,
                                      nbr_pulse_per_scan=nbr_pulse_per_scan, nbr_targets=nbr_targets,
                                      date_string=date_string, n_components_pca=n_components_pca, n_components_nnpca=n_components_nnpca)
    else:
        results_df = shallow_pca(AD_name=AD_name, shallow_name=shallow_name, dataset=dataset, representation=representation, nbr_modes=nbr_modes,
                                      pca_explained_var=pca_explained_var, nbr_seeds=nbr_seeds, train_ratio=train_ratio,
                                      nbr_pulse_per_scan=nbr_pulse_per_scan, nbr_targets=nbr_targets,
