...
def deep_svdd_ssl(AD_name, dataset, nbr_modes, nbr_epochs, batchsize, lr_init, lr_decay, lr_milestones,
                      weight_decay, nbr_seeds, train_ratio, nbr_pulse_per_scan, nbr_targets, date_string,
                  supervision="AD", loss="deepsvdd", arch="net0", nbr_modes_SAD=1, SAD_ratio=0.01):
    ...
    for exp_index, seed in enumerate(range(nbr_seeds)):
        ...
        c = init_ssl_sad_centers_c(device, train_loader, net)

        title = AD_name + " / " + "supervision:{}".format(supervision) + " / " + loss + " / " + "SAD_cls: {}".format(SAD_cls) + " / " + "SAD_ratio: {}".format(SAD_ratio)
        get_2D_latent_representations_trainloader(train_loader, net, device, title + " / train data before training", figure_path=results_path, figure_name="latent2Dtrain_untrained_seed{}_normal{}.png".format(seed, normal_cls), plot_centroids=True, centroids=c, supervision=supervision)
        get_2D_latent_representations_testloader(test_loader, net, device, title + " / test data before training", figure_path=results_path, figure_name="latent2Dtest_untrained_seed{}_normal{}.png".format(seed, normal_cls), plot_centroids=True, centroids=c, supervision=supervision)

        epoch_losses, trainAUCs, valAUCs, testAUCs, test_scores, test_labels, net = training_deepSVDD(train_loader, complete_train_loader, val_loader,
                                                                       test_loader, normal_cls, net, device, c,
        ...
        get_2D_latent_representations_trainloader(train_loader, net, device, title + " / train data after training", figure_path=results_path, figure_name="latent2Dtrain_trained_seed{}_normal{}.png".format(seed, normal_cls), plot_centroids=True, centroids=c, supervision=supervision)
        get_2D_latent_representations_testloader(test_loader, net, device, title + " / test data after training", figure_path=results_path, figure_name="latent2Dtest_trained_seed{}_normal{}.png".format(seed, normal_cls), plot_centroids=True, centroids=c, supervision=supervision)
...
if __name__ == '__main__':
    ...
    nbr_modes_SAD = int(sys.argv[17])
    SAD_ratio = float(sys.argv[18])
    experiment_name = str(sys.argv[19])
    ...
    results_df = deep_svdd_ssl(AD_name=AD_name, dataset=dataset, nbr_modes=nbr_modes, nbr_epochs=nbr_epochs, batchsize=batchsize,
                               ...
                               nbr_modes_SAD=nbr_modes_SAD, SAD_ratio=SAD_ratio)
